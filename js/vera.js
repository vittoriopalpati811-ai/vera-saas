...
const VSME_MODULES_ALL = [
  ...
  { code:'B1',    label:'B1 – Informazioni di base, forma giuridica e geolocalizzazione siti', optional:false },
  { code:'B2-E1', label:'B2/B3 – Pratiche di sostenibilità + Energia e Emissioni GHG (Scope 1/2)', optional:false },
  { code:'B2-E2', label:'Energia — Consumi (da file template VERA)', optional:false },
  { code:'B2-E3', label:'B7 – Rifiuti e gestione risorse (già B2-E3)', optional:false },
  { code:'B2-E4', label:'Trasporti — Logistica e mobilità', optional:false },
  { code:'B2-E5', label:'B5 – Biodiversità e siti in aree sensibili', optional:false },
  { code:'B3-S1', label:'B8/B9/B10 – Forza lavoro, salute/sicurezza, retribuzione e formazione', optional:false },
  ...
  { code:'B4-G',  label:'B11/C9 – Corruzione, sanzioni e diversità governance', optional:false },
  { code:'B5',    label:'Rischi e opportunità ESG — Analisi e mitigazione', optional:false },
  // ── MODULO C (volontario) ────────────────────────────────
  { code:'C1',    label:'B6 – Acqua — prelievo, consumo e scarico', optional:true },
  { code:'C2',    label:'B4 – Inquinamento aria, acqua e suolo', optional:true },
  { code:'C3',    label:'B7ext – Economia circolare e flussi materiali', optional:true },
  ...
  { code:'C1-Strategy', label:'C1 – Strategia: Modello di business e iniziative ESG (Comprehensive)', optional:true },
  { code:'C4-ClimRisk', label:'C4 – Rischi climatici fisici e di transizione (Comprehensive)', optional:true },
  { code:'C6-HRPolicy', label:'C6 – Politiche e processi sui diritti umani (Comprehensive)', optional:true },
  { code:'C7-HRInc',    label:'C7 – Incidenti sui diritti umani (Comprehensive)', optional:true },
  { code:'C8-Revenue',  label:'C8 – Ricavi da attività controverse (Comprehensive)', optional:true },
  // ── MODULO C condizionale (EFRAG VSME 2023 — applicabilità dipende dal profilo azienda) ──
  { code:'C-Climate', label:'C3 – Target riduzione GHG e piano di transizione climatica', optional:true, condition: (c) => !!(c && (c.hasNetZeroTarget || c.hasGreenFinancing)), conditionLabel:'Applicabile se: obiettivi net-zero o finanziamenti green' },
...
const VSME_QUESTIONS = {
  ...
  'B1': [
    ...
    { id:'vsme_legal_form', label:'Forma giuridica', type:'select', required:true,
      options:['S.r.l.','S.r.l.s.','S.p.A.','S.a.p.a.','S.n.c.','S.a.s.','Ditta individuale','Cooperativa','Altro'] },
    { id:'vsme_nace',       label:'Codice NACE UE (classificazione attività economica)', type:'text', required:true,
      placeholder:'es. C.25 (Fabbricazione di prodotti in metallo) — vedere ec.europa.eu/eurostat/ramon' },
    { id:'vsme_balance_sheet', label:'Totale attivo — stato patrimoniale (€)', type:'number',
      placeholder:'es. 2500000', hint:'Valore al 31/12 dell\'anno di rendicontazione (B1 VSME 2026 §27e-iii)' },
    { id:'vsme_turnover', label:'Ricavi/Fatturato dell\'anno (€)', type:'number',
      placeholder:'es. 4800000', hint:'Fatturato totale anno di rendicontazione (B1 VSME 2026 §27e-iv)' },
    { id:'vsme_primary_country', label:'Paese principale di operatività', type:'text', required:true,
      placeholder:'es. Italia', hint:'Paese di principale svolgimento delle attività (B1 §27e-vi)' },
    { id:'vsme_sites_geo', label:'Geolocalizzazione dei siti — stabilimenti, uffici, magazzini', type:'textarea',
      placeholder:'Un sito per riga: Nome sito — Via/Indirizzo completo\nes. Sede legale — Via delle Industrie 12, 31100 Treviso TV\nStabilimento Nord — Via Po 5, 10100 Torino TO',
      hint:'Necessario per B1 §27e-vii VSME 2026 — serve per la mappa siti e la verifica biodiversità' },
    { id:'vsme_cert', label:'Certificazioni e label di sostenibilità ottenuti (opzionale)', type:'textarea',
      placeholder:'es. ISO 14001:2015 — rilasciata da Bureau Veritas — valida fino 12/2026\nISO 45001 — rilasciata da DNV — rating A',
      hint:'Indicare: nome certificazione — ente certificatore — data rilascio/scadenza (B1 §28 VSME 2026)' },
  ],
  ...
  'B3-S1': [
    ...
    { id:'vsme_train_hrs_m', label:'Ore di formazione per dipendenti uomini (totale)', type:'number', placeholder:'es. 720' },
  ...
  'B4-G': [
    ...
    { id:'vsme_convictions_n',  label:'Condanne penali per corruzione o concussione nell\'anno (numero)', type:'number',
      placeholder:'0 se nessuna', required:true, hint:'B11 VSME 2026 §43 — solo condanne penali definitie' },
    { id:'vsme_fines_amount',   label:'Totale sanzioni pecuniarie per violazioni anti-corruzione (€)', type:'number',
      placeholder:'0 se nessuna', hint:'Include sanzioni amministrative e penali (B11 VSME 2026 §43)' },
    { id:'board_total',         label:'Componenti dell\'organo di governance (CdA / Collegio) — numero totale', type:'number',
      placeholder:'es. 3', hint:'C9 VSME 2026 §64 — include consiglieri esecutivi e non esecutivi' },
    { id:'board_f',             label:'Di cui: componenti donne', type:'number' },
    { id:'board_m',             label:'Di cui: componenti uomini', type:'number' },
    { id:'vsme_board_indep',    label:'Di cui: consiglieri indipendenti', type:'number' },
  ],
  ...
  /* ── C1-Strategy: Modello di business e strategia ESG ────────────────── */
  'C1-Strategy': [
    { id:'vsme_products',    label:'Descrizione dei principali prodotti/servizi offerti', type:'textarea', required:true,
      placeholder:'es. Produzione e distribuzione di componenti in acciaio per settore automotive e edilizia',
      hint:'C1 VSME 2026 §46a — gruppi significativi di prodotti/servizi' },
    { id:'vsme_markets',     label:'Mercati principali in cui opera l\'organizzazione', type:'textarea', required:true,
      placeholder:'es. B2B manifatturiero — prevalentemente Italia (70%) e Germania (20%) — canale diretto e distributori',
      hint:'C1 §46b — tipo di mercato (B2B/B2C/wholesale/retail) e geografie' },
    { id:'vsme_key_relations',label:'Principali relazioni commerciali (fornitori chiave, clienti, canali distributivi)', type:'textarea',
      placeholder:'es. Fornitori: acciaio (3 fornitori principali, di cui 2 italiani, 1 tedesco)\nClienti: 5 OEM (80% fatturato)',
      hint:'C1 §46c VSME 2026' },
    { id:'strategy_statement',label:'Elementi chiave della strategia relativi alla sostenibilità', type:'textarea',
      placeholder:'es. Obiettivo di carbon neutrality entro 2035, piano di efficienza energetica degli impianti, politica zero-waste',
      hint:'C1 §46d — solo se la strategia ha elementi rilevanti per la sostenibilità' },
    { id:'vsme_esg_budget',   label:'Risorse finanziarie/umane dedicate alla sostenibilità (opzionale)', type:'textarea',
      placeholder:'es. Responsabile ESG a tempo pieno + budget €50.000/anno per certificazioni e audit' },
  ],

  /* ── C4-ClimRisk: Rischi climatici ─────────────────────────────────────── */
  'C4-ClimRisk': [
    { id:'c4cr_has_risks', label:'L\'organizzazione ha identificato rischi climatici fisici o di transizione?',
      type:'select', required:true, options:['Sì — vedere dettaglio sotto','No — nessun rischio significativo identificato','Non ancora valutato'] },
    { id:'c4cr_hazards',   label:'Rischi climatici fisici identificati (alluvioni, ondate calore, siccità, ecc.)', type:'textarea',
      placeholder:'es. Rischio alluvioni — stabilimento in zona classificata R3\nOndate di calore — impatti su raffreddamento impianti',
      hint:'C4 VSME 2026 §56a — elencare i pericoli fisici rilevanti' },
    { id:'c4cr_transition',label:'Rischi di transizione climatica (politiche, normativa, tecnologia, mercato)', type:'textarea',
      placeholder:'es. Carbon pricing / ETS — potenziale aumento costi energetici\nNormativa EU sull\'efficienza energetica degli edifici — investimenti capex necessari',
      hint:'C4 §56a — rischi di transizione verso economia low-carbon' },
    { id:'c4cr_exposure',  label:'Come è stata valutata l\'esposizione e sensibilità di asset/attività ai rischi', type:'textarea',
      placeholder:'es. Mappatura fisica siti rispetto a zone alluvionali (fonte: ISPRA)\nAnalisi dipendenza da forniture soggette a stress idrico',
      hint:'C4 §56b VSME 2026' },
    { id:'c4cr_horizon',   label:'Orizzonti temporali dei rischi climatici identificati', type:'select',
      options:['Breve termine (<1 anno)','Medio termine (1-5 anni)','Lungo termine (>5 anni)','Multi-orizzonte'],
      hint:'C4 §56c — classificare i rischi per orizzonte temporale' },
    { id:'c4cr_adaptation',label:'Sono state adottate azioni di adattamento climatico?', type:'select', required:true,
      options:['Sì — vedere descrizione sotto','In corso di adozione','No'] },
    { id:'c4cr_adapt_desc',label:'Descrizione delle azioni di adattamento climatico adottate', type:'textarea',
      placeholder:'es. Installazione barriere anti-allagamento stabilimento Nord\nSistema di raffreddamento adiabatico per ridurre dipendenza da chiller',
      hint:'C4 §56d VSME 2026' },
    { id:'c4cr_severity',  label:'Valutazione complessiva dell\'esposizione al rischio climatico', type:'select',
      options:['Bassa','Media','Elevata','Non ancora valutata'],
      hint:'C4 §57 — indicare se rischio è alto, medio o basso' },
  ],

  /* ── C6-HRPolicy: Diritti umani — Politiche e processi ────────────────── */
  'C6-HRPolicy': [
    { id:'c6_code_conduct', label:'L\'organizzazione ha un codice di condotta o politica sui diritti umani per la propria forza lavoro?',
      type:'select', required:true, options:['Sì','No'],
      hint:'C6 VSME 2026 §61a — necessario per org. con >10 dipendenti' },
    { id:'c6_covers_child',   label:'La politica/codice copre: lavoro minorile?', type:'select',
      options:['Sì','No','Non applicabile'] },
    { id:'c6_covers_forced',  label:'La politica/codice copre: lavoro forzato?', type:'select',
      options:['Sì','No','Non applicabile'] },
    { id:'c6_covers_traffic', label:'La politica/codice copre: tratta di esseri umani?', type:'select',
      options:['Sì','No','Non applicabile'] },
    { id:'c6_covers_discrim', label:'La politica/codice copre: discriminazione?', type:'select',
      options:['Sì','No','Non applicabile'] },
    { id:'c6_covers_safety',  label:'La politica/codice copre: prevenzione degli infortuni?', type:'select',
      options:['Sì','No','Non applicabile'] },
    { id:'c6_covers_other',   label:'La politica copre altri aspetti rilevanti? (specificare)', type:'textarea',
      placeholder:'es. protezione della privacy, libertà di associazione, parità di genere' },
    { id:'c6_complaints_mech',label:'L\'organizzazione ha un meccanismo di gestione dei reclami per la propria forza lavoro?',
      type:'select', required:true, options:['Sì','No'],
      hint:'C6 §61c VSME 2026 — necessario per org. con >10 dipendenti' },
    { id:'c6_policy_url',     label:'Link o riferimento alla politica sui diritti umani (opzionale)', type:'text',
      placeholder:'es. https://www.azienda.it/politica-diritti-umani.pdf' },
  ],

  /* ── C7-HRInc: Incidenti sui diritti umani ─────────────────────────── */
  'C7-HRInc': [
    { id:'c7_inc_child',   label:'Incidenti confermati nella forza lavoro propria — lavoro minorile', type:'select', required:true,
      options:['No','Sì'],
      hint:'C7 VSME 2026 §62a-i — necessario per org. con >10 dipendenti' },
    { id:'c7_inc_forced',  label:'Incidenti confermati — lavoro forzato', type:'select', required:true,
      options:['No','Sì'] },
    { id:'c7_inc_traffic', label:'Incidenti confermati — tratta di esseri umani', type:'select', required:true,
      options:['No','Sì'] },
    { id:'c7_inc_discrim', label:'Incidenti confermati — discriminazione', type:'select', required:true,
      options:['No','Sì'] },
    { id:'c7_inc_other',   label:'Altri incidenti confermati (specificare se Sì)', type:'select',
      options:['No','Sì — vedere descrizione'] },
    { id:'c7_inc_desc',    label:'Descrizione degli incidenti e azioni adottate (se presenti)', type:'textarea',
      placeholder:'Descrivere natura dell\'incidente, numero di persone coinvolte e misure correttive adottate',
      hint:'C7 §62b — facoltativo ma raccomandato' },
    { id:'c7_vc_incidents',label:'L\'organizzazione è a conoscenza di incidenti confermati nella catena del valore, comunità, consumatori?',
      type:'select', required:true, options:['No','Sì — vedere descrizione'],
      hint:'C7 §62c VSME 2026 — necessario per org. con >10 dipendenti' },
    { id:'c7_vc_inc_desc', label:'Descrizione incidenti nella catena del valore/comunità/consumatori', type:'textarea',
      placeholder:'Specificare: tipologia, soggetti coinvolti, azioni correttive' },
  ],

  /* ── C8-Revenue: Ricavi da attività controverse ─────────────────────────── */
  'C8-Revenue': [
    { id:'c8_weapons',      label:'L\'organizzazione genera ricavi da armi controverse (mine antiuomo, munizioni a grappolo, armi chimiche/biologiche/nucleari)?',
      type:'select', required:true, options:['No','Sì — specificare'],
      hint:'C8 VSME 2026 §63a — Reg. UE 2020/1818 Art. 12(1)' },
    { id:'c8_weapons_amt',  label:'Ricavi da armi controverse (€)', type:'number', placeholder:'0' },
    { id:'c8_tobacco',      label:'L\'organizzazione genera ricavi dalla coltivazione o produzione di tabacco?',
      type:'select', required:true, options:['No','Sì — specificare'] },
    { id:'c8_tobacco_amt',  label:'Ricavi da tabacco (€)', type:'number', placeholder:'0' },
    { id:'c8_fossil',       label:'L\'organizzazione genera ricavi dal settore fossile (esplorazione, estrazione, lavorazione di carbone, petrolio o gas)?',
      type:'select', required:true, options:['No','Sì — specificare'] },
    { id:'c8_fossil_coal',  label:'Ricavi da carbone (€)', type:'number', placeholder:'0' },
    { id:'c8_fossil_oil',   label:'Ricavi da petrolio (€)', type:'number', placeholder:'0' },
    { id:'c8_fossil_gas',   label:'Ricavi da gas naturale (€)', type:'number', placeholder:'0' },
    { id:'c8_chemicals',    label:'L\'organizzazione produce pesticidi o altri prodotti agrochimici?',
      type:'select', required:true, options:['No','Sì — specificare'] },
    { id:'c8_chemicals_amt',label:'Ricavi da produzione pesticidi/agrofarmaci (€)', type:'number', placeholder:'0' },
  ],
};
